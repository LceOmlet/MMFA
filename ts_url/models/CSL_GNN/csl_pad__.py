...
__all__ = ["LearningShapeletsModelMixDistances_"]
...
class LearningShapeletsModelMixDistances_(nn.Module):
    ...
    def __init__(self, feat_dim=1, num_classes=None, max_len=224, dist_measure='mix',
        ...
        super(LearningShapeletsModelMixDistances_, self).__init__()
        ...
        if output_dim % 8 != 0:
            raise RuntimeError(f"output size: {output_dim}, should be devided by 8.")
        dim_out = output_dim
        ...
        self.projector = Projector("4096-8192", dim_out)

        if self.to_cuda:
            self.cuda()

    def forward(self, x, train=False, masking=False):
        x = x
