...
class MnasNet(nn.Module):
    def __init__(self, output_size=320, input_size=224, width_mult=1.):
        ...
        self.input_conv = nn.Sequential(Conv_3x3(1, input_channel, 2), SepConv_3x3(input_channel, 16))
        self.features = []
        input_channel = 16
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(),
            nn.Linear(self.last_channel, output_size)
        )
    ...
    def forward(self, x, **kwargs):
        batch, channel, n,n = x.shape
        x = x.reshape(batch * channel,1, n, n)
        x = self.input_conv(x)
        n = x.shape[-1]
        x = x.reshape(batch, channel, -1, n, n).mean(1)
        x = self.features(x)
...
if __name__ == '__main__':
    net = MnasNet()
    x_image = Variable(torch.randn(2, 3, 224, 224))
    y = net(x_image)
    print(y.shape)