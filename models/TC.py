...
class TC(nn.Module):
    def __init__(self, configs, device):
        ...
        self.Wk = nn.Linear(configs.TC.hidden_dim, self.num_channels)
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        # print(z_aug2.shape)zx

        batch = z_aug1.shape[0]
        # timestep = seq_len * 2 // 3
        # print(seq_len)
        t_samples = torch.randint(seq_len - timestep, size=(1,)).long().to(self.device)  # randomly pick time stamps

        nce = 0  # average over timestep and batch
        encode_samples = torch.empty((timestep, batch, self.num_channels)).float().to(self.device)
        ...
        pred = torch.empty((timestep, batch, self.num_channels)).float().to(self.device)
