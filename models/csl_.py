...
class MinEuclideanDistBlock(nn.Module):
    ...
    def __init__(self, shapelets_size, num_shapelets, in_channels=3, to_cuda=False):
...
class LearningShapeletsModelMixDistances(nn.Module):
    ...
    def forward(self, x, train_mode="", masking=False):
        ...
        [out[:, length_i * num_shapelet_per_length: (length_i + 1) * num_shapelet_per_length] for length_i in range(num_shapelet_lengths)]
